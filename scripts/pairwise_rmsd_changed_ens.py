...
def calc_selection_rmsd_with_ram(vars):
    """Calculates the RMSD between two selections of atoms.
    ...
    Returns:
        list: List of tuples with the RMSD values.
    """
    result = _calc_selection_rmsd(*vars)
    print(f"Step {args[0]} | TOTAL_RAM={total_ram_gb():.2f} GB", flush=True)
    return result
...
def split_in_batches(samples, n_jobs):
    batches = []
...
def batch_calc_selection_rmsd(batch, index):
    batch_rmsds = []
    for sample in batch:
        sample_rmsd = alc_selection_rmsd(sample[0], sample[1], sample[2])
        batch_rmsd.append(samples_rmsd)
    return (index, batch_rmsd)

def print_memory_usage(tag=""):
    process = psutil.Process(os.getpid()).memory_info().rss / (1024**3)  # in GB
    return process
    # print(f"[{tag}] PID {process.pid} using {mem:.2f} GB RAM")
...
def calc_rmsd(models_path, rmsd_path, chain_1, chain_2, interface_cutoff=10., n_cores=mp.cpu_count(), type="interface"):
    ...
    #samples = [(index, xyzr_all, del_mask) for index in reversed(range(len(xyzr_all) - 1))]
    # batches = split_in_batches(samples, n_cores)
    # rmsd_list = Parallel(n_jobs=n_cores, verbose=1)(delayed(batch_calc_selection_rmsd)(batch, index for index, batch in enumerate(batches)))

    # Calc pairwise RMSDs of the current selection of atoms.
    xyzr_all.share_memory_()
    del_mask.share_memory_()

    with mp.get_context("spawn").Pool(n_cores) as pool:
        rmsd_lists = list(tqdm(
            pool.imap(
                func=calc_selection_rmsd_with_ram,
                # iterable=list(zip(reversed(range(len(xyzr_all) - 1)), repeat(xyzr_all), repeat(del_mask))),
                iterable= [(index, xyzr_all, del_mask) for index in reversed(range(len(xyzr_all) - 1))],
                chunksize=max(2, round(len(file_names) / (n_cores * 16)))
            ),
    ...
    #     pbar = tqdm(it, total=len(file_names) - 1, desc="Calculating RMSDs")

    #     for i, result in enumerate(pbar):
    #         rmsd_lists.append(result)

    #         # update tqdm bar line
    #         pbar.set_postfix_str(f"RAM={ram_gb():.2f} GB")

    #         # ALSO print a real line immediately (so you still see it if it crashes)
    #         print(f"Step {i} | RAM={ram_gb():.2f} GB", flush=True)

    # with mp.get_context("spawn").Pool(n_cores) as pool:
    #     for i, result in enumerate(tqdm(pool.imap(
    #             func=calc_selection_rmsd,
    #             iterable=[(index, xyzr_all, del_mask) for index in reversed(range(len(xyzr_all) - 1))],
    #             chunksize=max(2, round(len(file_names) / (n_cores * 16)))
    #         ), total=len(file_names) - 1, desc="Calculating RMSDs")):

    #         # Print memory usage at each iteration
    #         print_memory_usage(tag=f"Step {i}")
    torch.cuda.empty_cache()

    # Reverse the order of the list.
    print("Processing RMSDs...")
    # rmsd_list = sorted(rmsd_lists, key=lambda x: x[0])
    # rmsd_list = [x[1] for x in rmsd_list]
